(function (global, factory) {
	...
	(global.LayerSwitcher2 = factory(global.ol.control.Control,global.ol.Observable,global.ol.layer.Group));
...
/**
 * OpenLayers LayerSwitcher2 Control, displays a list of layers and groups
 * associated with a map which have a `title` property.
 *
 * To be shown in the LayerSwitcher2 panel layers must have a `title` property;
 * base map layers should have a `type` property set to `base`. Group layers
 * (`LayerGroup`) can be used to visually group layers together; a group
 * with a `fold` property set to either `'open'` or `'close'` will be displayed
 * with a toggle.
 *
 * See [BaseLayerOptions](#baselayeroptions) for a full list of LayerSwitcher2
 * properties for layers (`TileLayer`, `ImageLayer`, `VectorTile` etc.) and
 * [GroupLayerOptions](#grouplayeroptions) for group layer (`LayerGroup`)
 * LayerSwitcher2 properties.
 *
 ...
 * })
 * // Specify a title property which will be displayed by the layer switcher
 * lyr.set('title2', 'OpenStreetMap');
 * ```
 *
 * To create a LayerSwitcher2 and add it to a map, create a new instance then pass it to the map's [`addControl` method](https://openlayers.org/en/latest/apidoc/module-ol_Map-Map.html#addControl).
 * ```javascript
 ...
 * @param opt_options LayerSwitcher2 options, see  [LayerSwitcher2 Options](#options) and [RenderOptions](#renderoptions) which LayerSwitcher2 `Options` extends for more details.
 */
class LayerSwitcher2 extends Control {
    constructor(opt_options) {
        ...
        this.collapseTipLabel = options.collapseTipLabel
            ...
            : 'Collapse legend';
        this.groupSelectStyle = LayerSwitcher2.getGroupSelectStyle(options.groupSelectStyle);
        ...
        LayerSwitcher2.enableTouchScroll_(this.panel);
    ...
    showPanel() {
         ...
         * });
         * @event LayerSwitcher2#show
         */
        this.dispatchEvent('show');
    }
    /**
     * Hide the layer panel. Fires `'hide'` event.
     * @fires LayerSwitcher2#hide
     */
    hidePanel() {
        if (this.element.classList.contains(this.shownClassName)) {
    ...
    updateButton() {
        if (this.element.classList.contains(this.shownClassName)) {
            ...
            this.button.setAttribute('title2', this.collapseTipLabel);
            this.button.setAttribute('aria-label', this.collapseTipLabel);
        }
        else {
            this.button.textContent = this.label;
            this.button.setAttribute('title2', this.tipLabel);
    ...
    static setGroupVisibility(map) {
        ...
        const groups = LayerSwitcher2.getGroupsAndLayers(map, function (l) {
            return (l instanceof LayerGroup &&
                !l.get('combine') &&
                !LayerSwitcher2.isBaseGroup(l));
        }).reverse();
        // console.log(groups.map(g => g.get('title2')));
        groups.forEach(function (grp) {
            // TODO Can we use getLayersArray, is it public in the esm build?
            const descendantVisibility = grp.getLayersArray().map(function (l) {
                const state = l.getVisible();
                // console.log('>', l.get('title2'), state);
                return state;
            });
    ...
    /**
     * **_[static]_** - Get an Array of all layers and groups displayed by the LayerSwitcher2 (has a `'title2'` property)
    ...
    static setVisible_(map, lyr, visible, groupSelectStyle) {
        // console.log(lyr.get('title2'), visible, groupSelectStyle);
    ...
    static renderLayer_(map, lyr, idx, options, render) {
        ...
        const lyrTitle = lyr.get('title2');
        const checkboxId = LayerSwitcher2.uuid();
        const label = document.createElement('label');
        if (lyr instanceof LayerGroup && !lyr.get('combine')) {
            const isBaseGroup = LayerSwitcher2.isBaseGroup(lyr);
    ...
    static forEachRecursive(lyr, fn) {
        lyr.getLayers().forEach(function (lyr, idx, a) {
            fn(lyr, idx, a);
            if (lyr instanceof LayerGroup) {
                LayerSwitcher2.forEachRecursive(lyr, fn);
            }
        });
    }
    /**
...
// Expose LayerSwitcher2 as ol.control.LayerSwitcher2 if using a full build of
// OpenLayers
if (window['ol'] && window['ol']['control']) {
    window['ol']['control']['LayerSwitcher2'] = LayerSwitcher2;
}
